...
static void rebuild_list(void) {
    ...
    for (int i = 0; i < inbox_count; i++) {
        ...
        lv_obj_set_size(row, 920, 90);
        lv_obj_set_pos(row, 0, i * 100);
        ...
        lv_obj_set_style_text_font(lbl_subj, &lv_font_montserrat_18, 0);
        lv_label_set_text(lbl_subj, subj);
        lv_obj_align(lbl_subj, LV_ALIGN_TOP_LEFT, 24, 0);

        /* text body */
        lv_obj_t * lbl_text = lv_label_create(row);
        lv_obj_set_style_text_color(lbl_text, lv_color_hex(0xffffff), 0);
        lv_obj_set_style_text_font(lbl_text, &lv_font_montserrat_22, 0);
        lv_obj_set_width(lbl_text, 720);
        lv_label_set_long_mode(lbl_text, LV_LABEL_LONG_DOT);
        lv_label_set_text(lbl_text, m->text);
        lv_obj_align(lbl_text, LV_ALIGN_TOP_LEFT, 24, 26);
...
void screen_inbox_show(void) {
    ...
    lv_obj_set_size(list_panel, 960, 480);
    lv_obj_set_pos(list_panel, 30, 100);
